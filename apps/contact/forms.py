from django import forms

CONTACT_EMAIL_SUBJECT = "Contact - Portfolio"


class ContactForm(forms.Form):
    name = forms.CharField(max_length=120)
    email = forms.EmailField()
    message = forms.CharField(widget=forms.Textarea)
    company = forms.CharField(required=False, widget=forms.HiddenInput)
    cf_turnstile_response = forms.CharField(required=False, widget=forms.HiddenInput)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        for field in self.fields.values():
            if field.widget.__class__.__name__ == "HiddenInput":
                continue
            current = field.widget.attrs.get("class", "")
            field.widget.attrs["class"] = f"{current} form-control".strip()
        self.fields["name"].widget.attrs.setdefault("placeholder", "Your name")
        self.fields["name"].widget.attrs.setdefault("autocomplete", "name")
        self.fields["email"].widget.attrs.setdefault("placeholder", "you@example.com")
        self.fields["email"].widget.attrs.setdefault("autocomplete", "email")
        self.fields["message"].widget.attrs.setdefault("rows", 3)
        self.fields["message"].widget.attrs.setdefault("style", "height: 6.5rem;")
        self.fields["message"].widget.attrs.setdefault(
            "placeholder",
            "Tell me about your project or role.",
        )

    def clean_company(self):
        value = self.cleaned_data.get("company", "").strip()
        if value:
            raise forms.ValidationError("Spam detected.")
        return value

    def clean_name(self):
        name = " ".join(self.cleaned_data["name"].strip().split())
        if len(name) < 2:
            raise forms.ValidationError("Please provide your full name.")
        return name

    def clean_email(self):
        return self.cleaned_data["email"].strip().lower()

    def clean_message(self):
        message = self.cleaned_data["message"].strip()
        if len(message.split()) < 5:
            raise forms.ValidationError(
                "Please include at least 5 words so I can respond with context."
            )
        if message.lower().count("http://") + message.lower().count("https://") > 3:
            raise forms.ValidationError("Please reduce the number of links in your message.")
        return message


class ContactOtpForm(forms.Form):
    challenge_id = forms.CharField(max_length=64)
    otp = forms.CharField(max_length=12)

    def clean_otp(self):
        return self.cleaned_data["otp"].strip()

    def clean_challenge_id(self):
        return self.cleaned_data["challenge_id"].strip()
